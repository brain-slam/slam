...
#################################################
# MORE DETAILED AND GENERALIZED DISPLAY OPTIONS #
#################################################

# AUTOMATIZED COLORMAP DEPENDING ON NUMBER OF BANDS
# -------------------------------------------------
# Define automatically the sulci/gyri folding pattern colormap, in correlation with nlevels --> colors, colormap_sulci, colormap_gyri
number_of_high_folding_pattern_bands = int(nlevels/2) 

colors = np.zeros((2*number_of_high_folding_pattern_bands+1, 4)) # 4 colors for 7 bands (black + B4 + B5 + B6)
...
for i in range(number_of_high_folding_pattern_bands):
    colors[i+1+number_of_high_folding_pattern_bands, :] = np.array([1, color_variations_coef[i], 0, 1])
...
# -----------------------------
def plot_global_coefficients_and_bands_sulci_or_gyri(group_indices, coefficients, colormap):
...
grouped_spectrum_sulci, group_indices_sulci, coefficients_sulci, _ = spgy.spectrum(mean_curv_sulci, lap_b,
                                                              eigVects, eigVal)
...
grouped_spectrum_gyri, group_indices_gyri, coefficients_gyri, _ = spgy.spectrum(mean_curv_gyri, lap_b,
                                                              eigVects, eigVal)

# Plot mean curvature coefficients & compacted power spectrum characterizing either Sulci either Gyri folding pattern 
# -------------------------------------------------------------------------------------------------------------------
coefficients_colors_sulci = plot_global_coefficients_and_bands_sulci_or_gyri(group_indices_sulci, coefficients_sulci, colormap_sulci)
coefficients_colors_gyri = plot_global_coefficients_and_bands_sulci_or_gyri(group_indices_gyri, coefficients_gyri, colormap_gyri)

fig, axs = plt.subplots(2, 2)

# GLOBAL FOLDING PATTERN OF SULCI
#axs[0,0].plot(np.sqrt(eigVal/2*np.pi), coefficients_sulci, color=coefficients_colors_sulci)
axs[0,0].scatter(np.sqrt(eigVal/2*np.pi), coefficients_sulci, marker='+', s=10, linewidths=0.5, color=coefficients_colors_sulci)
#axs[0,0].scatter(np.sqrt(eigVal[1:]/2*np.pi), coefficients_sulci[1:], marker='+', s=10, linewidths=0.5, color=coefficients_colors_sulci[1:]) # remove B0 coefficient
axs[0,0].set_xlabel('Frequency (m⁻¹)')
axs[0,0].set_ylabel('Coefficients mean_curv<=0') 

axs[0,1].bar(np.arange(0, nlevels), grouped_spectrum_sulci.squeeze(), color=colormap_sulci)
#axs[0,1].bar(np.arange(1, nlevels), grouped_spectrum_sulci[1:].squeeze(), color=colormap_sulci[1:]) # remove B0
axs[0,1].set_xlabel('Spangy frequency bands')
axs[0,1].set_ylabel('Power spectrum mean_curv<=0')

# GLOBAL FOLDING PATTERN OF GYRI
#axs[1,0].plot(np.sqrt(eigVal/2*np.pi), coefficients_gyri, color=coefficients_colors_gyri)
axs[1,0].scatter(np.sqrt(eigVal/2*np.pi), coefficients_gyri, marker='+', s=10, linewidths=0.5, color=coefficients_colors_gyri)
#axs[1,0].scatter(np.sqrt(eigVal[1:]/2*np.pi), coefficients_gyri[1:], marker='+', s=10, linewidths=0.5, color=coefficients_colors_gyri[1:]) # remove B0 coefficient
axs[1,0].set_xlabel('Frequency (m⁻¹)')
axs[1,0].set_ylabel('Coefficients mean_curv>0') 

axs[1,1].bar(np.arange(0, nlevels), grouped_spectrum_gyri.squeeze(), color=colormap_gyri)
#axs[1,1].bar(np.arange(1, nlevels), grouped_spectrum_gyri[1:].squeeze(), color=colormap_gyri[1:]) # remove B0
...
limit = np.max(loc_dom_band) - number_of_high_folding_pattern_bands + 1 # limit band number under which band is displayed in black
...
p = pv.Plotter()
p.add_mesh(mesh, scalars=loc_dom_band, show_edges=False, cmap=localbands_colormap, show_scalar_bar=False)
