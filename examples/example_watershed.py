...
tex_labels, tex_pits, tex_ridges = swat.get_textures_from_dict(
    mesh, basins, ridges, save=True, outdir=path_to_output)

###############################################################################
# generate the sulcal graph
g = ssg.get_sulcal_graph(
    adjacency, basins, ridges, save=True, outdir=path_to_output)

###############################################################################
# generate the textures from graph
tex_labels, tex_pits, tex_ridges = (ssg.get_textures_from_graph
                                    (g, mesh, save=True, outdir=path_to_output))
