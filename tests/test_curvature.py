...
class TestCurvatureMethods(unittest.TestCase):
    ...
    def test_correctness_curvature_sphere(self):
        ...
        for i in [1, 2, 4]:
            ...
            precision_A = 0.000001
            precision_B = 0.000001
    ...
    def test_correctness_curvature_quadric(self):
        ...
        for i, v in enumerate(a):
            ...
            if len(v) > 4:
                print(
                    "{0:10} {1:5} {2:16} {3:2} {4} {5}".format(
                        v[0], v[1], v[2], "=", numeric_value, v[4]
                    )
                )
            else:
                print(
                    "{0:10} {1:5} {2:16} {3:2} {4}".format(
    ...
    def test_error_drop_curvature_sphere(self):
        ...
        for j in range(3):
            ...
            k_mean_estim = (k1_estim + k2_estim) * 0.5
            # k_gauss_estim = (k1_estim * k2_estim)
    ...
    def test_correctness_decomposition_sphere(self):
        ...
        precisionA = 0.0000001
        precisionB = 0.0000001

        radius = 3

        mesh_a = trimesh.creation.icosphere(subdivisions=2, radius=radius)
        ...
        assert (
            np.isclose(shapeIndex, 1, precisionA)
            | np.isclose(shapeIndex, -1, precisionA)
        ).all()
    ...
    def test_correctness_decomposition_quadric(self):
        ...
        for i in range(len(set_of_tests)):
            ...
            with self.subTest(i):
                ...
                # Generate quadric
                quadric = sgps.generate_quadric(
                    K, nstep=[20, 20], ax=3, ay=1, random_sampling=False, ratio=0.3,
                )
                ...
                ) | np.isclose(computed_shapeIndex, -correct_shape_index, precisionA)
                assert np.isclose(
                    computed_curvedness, correct_curvedness, precisionB
                ) | np.isclose(computed_curvedness, -correct_curvedness, precisionB)
    ...
    def test_correctness_direction_quadric(self):
        ...
        assert percentage_low_error > 0.75
